...
def main(args):
    ...
    def G_step(result):
        ...
        with timeit('loss', args.timing):
            ...
            crops_encoded_rec_loss = F.l1_loss(crops_pred_encoded, crops_encoded)
            total_loss = add_loss(total_loss, crops_encoded_rec_loss, losses, 'crops_encoded_rec_loss',
    ...
    while True:
        ...
        for step, batch in enumerate(tqdm(train_loader, desc='Training Epoch %d' % epoch, total=len(train_loader))):
            ...
            if t % (args.checkpoint_every * (args.n_critic + args.n_gen)) == 0:
                ...
                checkpoint['e_obj_state'] = all_in_one_model.obj_encoder.state_dict()
                checkpoint['e_obj_optim_state'] = all_in_one_model.optimizer_e_obj.state_dict()
