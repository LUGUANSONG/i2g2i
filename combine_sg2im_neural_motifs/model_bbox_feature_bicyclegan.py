...
def build_obj_encoder(args):
    if args.checkpoint_start_from is not None:
        checkpoint = torch.load(args.checkpoint_start_from)
        e_obj_kwargs = checkpoint['e_obj_kwargs']
        obj_encoder = Encoder(**e_obj_kwargs)
        raw_state_dict = checkpoint['e_obj_state']
        state_dict = {}
        for k, v in raw_state_dict.items():
            if k.startswith('module.'):
                k = k[7:]
    ...
    return obj_encoder, e_obj_kwargs
...
class neural_motifs_sg2im_model(nn.Module):
    def __init__(self, args, ind_to_classes):
        ...
        self.obj_encoder, e_obj_kwargs = build_obj_encoder(args)
        self.optimizer_e_obj = torch.optim.Adam(self.obj_encoder.parameters(), lr=args.learning_rate, betas=(0.5, 0.999))
        ...
        if restore_path is not None and os.path.isfile(restore_path):
            ...
            self.obj_encoder.load_state_dict(checkpoint['e_obj_state'])
            self.optimizer_e_obj.load_state_dict(checkpoint['e_obj_optim_state'])
            ...
            checkpoint = {
                ...
                'e_obj_state': None, 'e_obj_best_state': None, 'e_obj_optim_state': None,
                'best_t': [],
            }
        ...
        if self.forward_G:
            with timeit('generator forward', self.args.timing):
                if self.training:
                    ...
                    z_random = torch.randn((obj_fmaps_random.shape[0], self.args.object_noise_dim),
                                           dtype=obj_fmaps_random.dtype, device=obj_fmaps_random.device)
                    ...
                    crops_pred_encoded = crop_bbox_batch(imgs_pred_encoded, boxes_encoded, obj_to_img_encoded,
                                                         self.args.crop_size)

                    crops_pred_random = crop_bbox_batch(imgs_pred_random, boxes_random, obj_to_img_random,
                                                        self.args.crop_size)
                    ...
                    z_random = torch.randn((obj_fmaps.shape[0], self.args.object_noise_dim),
                                           dtype=obj_fmaps.dtype, device=obj_fmaps.device)
