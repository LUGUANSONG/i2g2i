...
def check_model(args, loader, model, output_path):
    ...
    with torch.no_grad():
        for _batch in loader:
            ...
            bg_layout = result.bg_layout.cpu()
            imgs_pred_list = torch.stack(imgs_pred_list)
            imgs_pred_means = imgs_pred_list.mean(dim=0)
            imgs_pred_stds = imgs_pred_list.std(dim=0)

            for k, image in enumerate(imgs_pred_means):
                ...
                for i, std in enumerate(stds):
                    if not len(std.shape) == 2:
                        print("std is not 2-dimensional", std.shape)
                        raise Exception("std is not 2-dimensional")

                    plt.imshow(std, cmap='viridis')
                    plt.colorbar()
