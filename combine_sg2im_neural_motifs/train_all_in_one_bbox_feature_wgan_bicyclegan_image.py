...
# combine
from model_bbox_feature_bicyclegan_image import neural_motifs_sg2im_model
...
def calculate_model_losses(args, img, img_pred, mask_noise_indexes=None, bg_layout=None):
    total_loss = torch.zeros(1).to(img)
    losses = {}

    # apply L1 loss only on background
    if bg_layout is not None and args.l1_on_bg:
        img = img * bg_layout
        img_pred = img_pred * bg_layout
...
def main(args):
    ...
    def G_step(result):
        ...
        with timeit('loss', args.timing):
            ...
            imgs_encoded_rec_loss = F.l1_loss(imgs_pred_encoded, imgs_encoded)
            total_loss = add_loss(total_loss, imgs_encoded_rec_loss, losses, 'imgs_encoded_rec_loss',
                                  args.imgs_encoded_rec_loss_weight)

            kl_loss = torch.sum(1 + logvar_encoded - mu_encoded.pow(2) - logvar_encoded.exp()) * (-0.5)
            total_loss = add_loss(total_loss, kl_loss, losses, 'kl_loss', args.kl_loss_weight)
            ...
            if criterionVGG is not None:
                ...
                if mask_noise_indexes is not None and args.perceptual_not_on_noise:
                    perceptual_loss = criterionVGG(preceptual_imgs_pred[mask_noise_indexes],
                                                    perceptual_imgs[mask_noise_indexes])
                else:
                    perceptual_loss = criterionVGG(preceptual_imgs_pred, perceptual_imgs)
        ...
        z_random_rec_loss = torch.mean(torch.abs(z_random_rec - z_random)) * args.z_random_rec_loss_weight
        all_in_one_model.optimizer.zero_grad()
        all_in_one_model.optimizer_e_img.zero_grad()
    ...
    while True:
        ...
        for step, batch in enumerate(tqdm(train_loader, desc='Training Epoch %d' % epoch, total=len(train_loader))):
            ...
            if t % (args.checkpoint_every * (args.n_critic + args.n_gen)) == 0:
                ...
                checkpoint['e_img_state'] = all_in_one_model.img_encoder.state_dict()
                checkpoint['e_img_optim_state'] = all_in_one_model.optimizer_e_img.state_dict()
                ...
                key_blacklist = ['model_state', 'optim_state', 'model_best_state',
                                 ...
                                 'e_img_state', 'e_img_optim_state', 'e_img_best_state']
                small_checkpoint = {}
