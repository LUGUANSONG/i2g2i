...
class Blob(object):
    def __init__(self, mode='det', is_train=False, num_gpus=1, primary_gpu=0, batch_size_per_gpu=3):
        ...
        #                        # to 0
        self.fmaps = []  # [num_gt, 4096] feature_map of each bbox
    ...
    def _chunkize(self, datom, tensor=torch.LongTensor):
        ...
        if isinstance(datom[0], np.ndarray):
            return Variable(tensor(np.concatenate(datom, 0)), volatile=self.volatile), chunk_sizes
        else:
            return Variable(torch.cat(datom, 0), volatile=self.volatile), chunk_sizes
    ...
    def reduce(self):
        ...
        self.fmaps, _ = self._chunkize(self.fmaps)
        # if self.is_train:
    ...
    def scatter(self):
        ...
        self.fmaps_primary = self.fmaps.cuda(self.primary_gpu, async=True)
        ...
        self.fmaps = self._scatter(self.fmaps, self.gt_box_chunks)
    ...
    def __getitem__(self, index):
        ...
        if index == 0 and self.num_gpus == 1:
            ...
            return self.imgs, image_offset, self.gt_boxes, self.gt_classes, self.fmaps
        ...
        return self.imgs[index], image_offset, self.gt_boxes[index], self.gt_classes[index], self.fmaps[index]
