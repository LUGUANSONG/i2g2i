...
def build_coco_dsets(args):
    ...
    print('There are %d iterations per epoch' % (num_imgs // args.batch_size))
...
def check_model(args, t, loader, model):
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            total_loss, losses = calculate_model_losses(args, skip_pixel_loss, model, imgs, imgs_pred,
...
def main(args):
    ...
    while True:
        if t >= args.num_iterations:
            break
        epoch += 1
        print(colored('Starting epoch %d' % epoch, "yellow"))
        ...
        for batch in tqdm(train_loader, desc="train epoch %d" % epoch, total=len(train_loader)):
            ...
            if len(batch) == 6:
                ...
                assert False
            # print(imgs.shape, objs.shape, boxes.shape, triples.shape, obj_to_img.shape, triple_to_img.shape)
            # print("objs", objs)
            # print("boxes", boxes)
            # print("triples", triples)
            # print("obj_to_img", obj_to_img)
            # print("triple_to_img", triple_to_img)
            # os._exit(0)
            predicates = triples[:, 1]
            ...
            with timeit('forward', args.timing):
                ...
                imgs_pred, boxes_pred, masks_pred, predicate_scores = model_out
                # print("model_masks.shape", "masks_pred.shape")
                # print(model_masks.shape, masks_pred.shape)
                # os._exit(0)
            ...
            if t % args.checkpoint_every == 0:
                ...
                for name, image in t_samples.items():
                    summary_writer.add_image("train_%s" % name, image, t)
                summary_writer.add_scalar("train_iou", t_avg_iou.item(), t)
                ...
                for name, image in val_samples.items():
                    summary_writer.add_image("val_%s" % name, image, t)
                summary_writer.add_scalar("val_iou", val_avg_iou.item(), t)

                print('train iou: ', t_avg_iou.item(), 'val iou: ', val_avg_iou.item())
                # print('val iou: ', val_avg_iou.item())
        ...
        print(colored("End of epoch [%d], take time [%.3f]s" % (epoch, time.time() - epoch_start_time), "yellow"))
